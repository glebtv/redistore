...
// NewRediStore - create a new redis store from redis client
func NewRediStore(client *redis.Client, keyPairs ...[]byte) (*RediStore, error) {
	rs := &RediStore{
		Client: client,
...
// ping run ping on underlying *redis.Client
func (s *RediStore) ping() (string, error) {
...
func (s *RediStore) Delete(r *http.Request, w http.ResponseWriter, session *sessions.Session) error {
	err := s.Client.Del(s.keyPrefix + session.ID).Err()
	if err != nil {
		return err
	}
...
func (s *RediStore) load(session *sessions.Session) (bool, error) {
	r := s.Client.Get(s.keyPrefix + session.ID)
	if r.Err() == redis.Nil {
	...
	return true, s.serializer.Deserialize(bytes, session)
}
